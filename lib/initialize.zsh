...
function _ZPM-load-plugin() {
  ...
  if [[ "$1"  == *",fpath-only"* ]]; then
    if [[ "$1"  == *",fpath-only:"* ]]; then
      ...
      _ZPM-addfpath "${Plugin_path}$(echo "$1" | awk -F'fpath-only:' '{print $2}' | awk -F',' '{print $1}')"
      return 0
    fi
  ...
  if [[ "$1"  == *",bin-only"* ]]; then
    if [[ "$1"  == *",bin-only:"* ]]; then
      ...
      _ZPM-addpath "${Plugin_path}$(echo "$1" | awk -F'bin-only:' '{print $2}' | awk -F',' '{print $1}')"
      return 0
    fi
  ...
  if [[ "$1"  == *",bin:"* ]]; then
    ...
    _ZPM-addpath "${Plugin_path}$(echo "$1" | awk -F'bin:' '{print $2}' | awk -F',' '{print $1}')"
  else
